    ...
    for (int i = 0; i < statuses.size(); i++) {
      ...
      if (batteryStatus.getDevice() == 0) {
        ...
        if (i < statuses.size() - 1) {
          ...
          if (
            nextStatus.getDate().getTime() / 60000 == batteryStatus.getDate().getTime() / 60000 &&
      ...
      try {
        cursor = db.query(
          ...
          new String[] {
            "timestamp",
            "charge_percent",
            "current_instant_milliamperes",
            "current_avg_milliamperes",
        ...
        while (cursor.moveToNext()) {
          statuses
            .add(new BatteryStatus.Builder(device)
    ...
    public List<BatteryStatus> export() {
      ...
      try {
        cursor = db.query(
          ...
          null,
          null,
          "timestamp DESC, device ASC"
        );

        ArrayList<BatteryStatus> statuses = new ArrayList<BatteryStatus>();
        while (cursor.moveToNext()) {
          statuses
            .add(new BatteryStatus.Builder(cursor.getInt(1))
            .timestamp(cursor.getLong(0))
